...
class EventSeriesListCreateView(APIView):
    ...
    @extend_schema(request=EventSeriesSerializer, responses={201: EventSeriesSerializer})
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return success_response(
                data=EventSeriesSerializer(series).data, message="Event series created", status=201
            )
...
class EventSeriesDetailView(APIView):
    ...
    def get(self, request, series_id):
        ...
        try:
            series = EventSeries.objects.prefetch_related("need_templates").get(pk=series_id)
    ...
    @extend_schema(request=EventSeriesSerializer, responses={200: EventSeriesSerializer})
    def patch(self, request, series_id):
        ...
        if serializer.is_valid():
            serializer.save()
            return success_response(data=EventSeriesSerializer(series).data, message="Event series updated")
        return error_response(message="Validation Error", errors=serializer.errors)
...
class EventSeriesOccurrencesView(APIView):
    ...
    def get(self, request, series_id):
        ...
        events = Event.objects.filter(series=series).select_related(
...
class EventSeriesGenerateView(APIView):
    ...
    def post(self, request, series_id):
        ...
        if not series.recurrence_rule:
            return error_response(message="No recurrence rule set on series", status=400)
        ...
        try:
            ...
            if generate_until_str:
                generate_until = datetime.fromisoformat(generate_until_str.replace("Z", "+00:00"))
        ...
        if not category and "category_id" in request.data:
            ...
            category = EventCategory.objects.filter(pk=request.data["category_id"]).first()
            
        if not category:
            return error_response(message="Cannot generate without a base category. Provide category_id or create a manual occurrence first.", status=400)

        generated_events = []
        with transaction.atomic():
            # Get the highest occurrence_index currently
            last_event = Event.objects.filter(series=series).order_by("-occurrence_index").first()
