...
from apps.events.models import (
    EventCategory, EventSeries, EventSeriesNeedTemplate, Event,
    EventMedia, EventInterest, EventLifecycleTransition,
    EventHighlight, EventReview, EventReviewMedia, EventVendorReview, EventView
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dj_service = VendorService.objects.create(
            vendor=users["vendor1"], title="DJ Frank's Beats", description="Premium DJ services for your events.",
            category="DJ", base_price=Decimal("500.00"), location_city="New York"
        )
        VendorService.objects.filter(pk=dj_service.pk).update(portfolio_image=PLACEHOLDER_PORTFOLIO)
        
        photo_service = VendorService.objects.create(
            vendor=users["vendor2"], title="Grace Photos", description="Capturing your best moments.",
            category="Photography", base_price=Decimal("300.00"), location_city="New York"
        )
        VendorService.objects.filter(pk=photo_service.pk).update(portfolio_image=PLACEHOLDER_PORTFOLIO)
        
        cater_service = VendorService.objects.create(
            vendor=users["vendor3"], title="Heidi's Catering", description="Delicious food for all.",
            category="Catering", base_price=Decimal("1000.00"), location_city="New York"
        )
        VendorService.objects.filter(pk=cater_service.pk).update(portfolio_image=PLACEHOLDER_PORTFOLIO)
        ...
        series1 = EventSeries.objects.create(
            host=users["host1"], name="Weekly Tech Meetup", description="Learn tech every week.",
            recurrence_rule="FREQ=WEEKLY;BYDAY=WE", default_location_name="Tech Hub",
            default_capacity=50, default_ticket_price_standard=Decimal("10.00")
        )
        EventSeriesNeedTemplate.objects.create(
            series=series1, title="Guest Speaker", category="Speaker", criticality="essential"
        )

        # 5. Events - Single & Recurrences representing all lifecycle states
        self.stdout.write("Creating events...")
        def create_event(title, host, status, l_state, start_offset_days, cat, series=None):
            start = now + timedelta(days=start_offset_days)
            end = start + timedelta(hours=3)
            e = Event.objects.create(
                host=host, title=title, description=f"Description for {title}",
                category=cat, location_name="The Venue", location_address="123 Main St",
                start_time=start, end_time=end, capacity=100,
                ticket_price_standard=Decimal("20.00") if status != 'draft' else None,
                status=status, lifecycle_state=l_state, series=series
            )
            Event.objects.filter(pk=e.pk).update(cover_image=PLACEHOLDER_EVENT_IMG)
            return e

        events = {
            "draft": create_event("Draft Party", users["host1"], "draft", "draft", 30, db_categories["nightlife"]),
            "published": create_event("Published Concert", users["host2"], "published", "published", 15, db_categories["music"]),
            "at_risk": create_event("At Risk Picnic", users["host1"], "published", "at_risk", 2, db_categories["outdoors-adventure"]),
            "postponed": create_event("Postponed Workshop", users["host2"], "published", "postponed", 45, db_categories["workshops-classes"]),
            "event_ready": create_event("Ready Festival", users["host1"], "published", "event_ready", 1, db_categories["festivals"]),
            "live": create_event("Live Comedy Show", users["host2"], "published", "live", 0, db_categories["comedy"]),
            "completed": create_event("Past Gala", users["host1"], "completed", "completed", -10, db_categories["networking-social"]),
            "series_past": create_event("Weekly Tech Meetup #1", users["host1"], "completed", "completed", -7, db_categories["tech-innovation"], series1),
            "series_upcoming": create_event("Weekly Tech Meetup #2", users["host1"], "published", "published", 7, db_categories["tech-innovation"], series1),
        }
        ...
        for key, e in events.items():
            if e.lifecycle_state != 'draft':
                EventLifecycleTransition.objects.create(
                    event=e, from_state="draft", to_state=e.lifecycle_state, actor=e.host, reason="Testing"
                )
        ...
        need1 = EventNeed.objects.create(
            event=events["published"], title="Looking for DJ", category="DJ", budget_max=Decimal("600.00")
        )
        app1 = NeedApplication.objects.create(
            need=need1, vendor=users["vendor1"], service=dj_service, proposed_price=Decimal("500.00"), status="accepted"
        )
        ...
        NeedInvite.objects.create(
            need=need2, vendor=users["vendor2"], invited_by=users["host2"], message="Please apply!"
        )
        ...
        for goer in [users["goer1"], users["goer2"], users["goer3"]]:
            Ticket.objects.create(event=events["published"], goer=goer, ticket_type="standard", price_paid=Decimal("20.00"))
            Ticket.objects.create(event=events["completed"], goer=goer, ticket_type="flexible", price_paid=Decimal("25.00"))
        ...
        h1 = EventHighlight.objects.create(
            event=events["completed"], author=users["goer1"], role="goer", text="Best gala ever!", moderation_status="approved"
        )
        EventHighlight.objects.filter(pk=h1.pk).update(media_file=PLACEHOLDER_EVENT_IMG)
        
        h2 = EventHighlight.objects.create(
            event=events["live"], author=users["host2"], role="host", text="We are live!", moderation_status="approved"
        )
        EventHighlight.objects.filter(pk=h2.pk).update(media_file=PLACEHOLDER_EVENT_IMG)

        # Event Media
        m1 = EventMedia.objects.create(event=events["completed"], media_type="image", category="highlight")
        EventMedia.objects.filter(pk=m1.pk).update(file=PLACEHOLDER_EVENT_IMG)

        # 9. Reviews
        self.stdout.write("Creating reviews...")
        rev = EventReview.objects.create(
            event=events["completed"], reviewer=users["goer2"], rating=5, text="Fantastic experience."
        )
        ...
        VendorReview.objects.create(
            vendor_service=dj_service, reviewer=users["goer3"], rating=5, text="Played all the right tracks."
        )
        ...
        req = EventRequest.objects.create(
            requester=users["goer1"], title="Bring back the Retro 80s Party!",
            description="We need more 80s synth music nights.",
            category=db_categories["music"], location_city="New York", status="open"
        )
        RequestUpvote.objects.create(request=req, user=users["goer2"])
        RequestWishlist.objects.create(request=req, user=users["vendor1"], wishlist_as="vendor")
        req.upvote_count = 1
        req.save()
