...
export function parseGoogleMapsContent(text: string): BusinessDetails {
  ...
  for (let i = 1; i < Math.min(filteredLines.length, 6); i++) {
    ...
    if (
      !result.category &&
      (line.includes('·') ||
        (i > 1 && !/^\d\.\d$/.test(line) && !/^\(\d+/.test(line) && line.length < 50))
    ) {
  ...
  if (allIndex !== -1) {
    ...
    for (let i = allIndex + 1; i < filteredLines.length; i++) {
      ...
      if (
        line.length > 1 &&
        (nextLine.includes('review') || nextLine.includes('Local Guide'))
      )
        break;
  ...
  if (reviewsIndex !== -1) {
    ...
    for (let i = reviewsIndex + 1; i < filteredLines.length; i++) {
      ...
      if (
        ...
        (filteredLines[i + 1].includes('review') ||
          filteredLines[i + 1].includes('Local Guide'))
      ) {
      ...
      if (line === '' && currentReview) {
        ...
        while (i + 1 < filteredLines.length) {
          ...
          if (
            nextLine.length > 1 &&
            (nextNextLine.includes('review') || nextNextLine.includes('Local Guide'))
          )
            break;
