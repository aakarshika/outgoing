...
export const PrivacySettingsSection = () => {
    ...
    const cardClass = 'border-2 border-gray-800 bg-white p-6 shadow-[3px_4px_0px_#333] relative';
    const cardBg = {
    ...
    const Toggle = ({ active, onToggle, label, icon: Icon, description }: any) => (
        <div className="flex items-center justify-between border-2 border-dashed border-gray-300 py-3 px-4 mb-4">
            <div className="flex items-center gap-4">
                <div className={`p-2 border-2 border-gray-800 ${active ? 'bg-green-100' : 'bg-gray-100'} shadow-[1px_1px_0px_#333]`}>
                    <Icon size={18} className={active ? 'text-green-600' : 'text-gray-400'} />
                </div>
                <div>
                    <p className="font-bold text-gray-800" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.2rem' }}>
                        {label}
                    </p>
                    <p className="text-gray-500 text-sm" style={{ fontFamily: '"Caveat", cursive', fontSize: '1rem' }}>
                        {description}
                    </p>
                </div>
            </div>
            <button
                onClick={onToggle}
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-gray-800 transition-colors duration-200 ease-in-out focus:outline-none ${active ? 'bg-green-400' : 'bg-gray-200'
                    }`}
            >
                <span
                    className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white border-2 border-gray-800 shadow-[1px_1px_0px_#333] transition duration-200 ease-in-out ${active ? 'translate-x-5' : 'translate-x-0'
                        }`}
    ...
    if (loading) {
        return (
            <div className="max-w-2xl space-y-6">
                <header>
                    <h2 className="text-2xl text-gray-900" style={{ fontFamily: '"Permanent Marker", cursive' }}>
                        Privacy Settings
                    </h2>
                    <p className="mt-1 text-gray-500" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.15rem' }}>
                        loading your secrets... 🤫
                    </p>
                </header>
                <div className={`${cardClass} flex items-center justify-center min-h-[200px]`} style={cardBg}>
                    <span className="text-gray-400 animate-pulse" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.2rem' }}>
                        Fetching settings...
                    </span>
                </div>
            </div>
    ...
    return (
        <div className="max-w-2xl space-y-8 pb-12">
            <header className="relative">
                ...
                />
                <h2 className="text-2xl text-gray-900" style={{ fontFamily: '"Permanent Marker", cursive', transform: 'rotate(-1deg)' }}>
                    Privacy Settings
                </h2>
                <p className="mt-1 text-gray-500" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.15rem' }}>
            ...
            <div className={cardClass} style={cardBg}>
                <div className="absolute -top-2 left-[30%] w-24 h-5 pointer-events-none" style={{ background: 'rgba(147, 197, 253, 0.5)', transform: 'rotate(2deg)', border: '1px solid rgba(0,0,0,0.05)' }} />
                <h3 className="mb-6 flex items-center gap-2 text-gray-900" style={{ fontFamily: '"Permanent Marker", cursive', fontSize: '1.2rem' }}>
            ...
            <div className="flex h-6 items-center justify-end gap-2 px-2">
                {saving && (
                    <span className="text-gray-500 animate-pulse" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.1rem' }}>
                        Saving settings...
                    </span>
                )}
                {lastSaved && !saving && (
                    <span className="text-green-600 flex items-center gap-1" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.1rem' }}>
