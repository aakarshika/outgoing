...
export function NormalBrowseModule({ event }: NormalBrowseModuleProps) {
  ...
  return (
    <Box sx={{ px: 2, pt: 2 }}>
      ...
      >
        {moreFromHost.length > 0 ? `More from ${event.host?.name || event.host?.username}` : 'More like this'}
      </Typography>
      ...
      >
        {events.map((evt: any, idx: number) => {
          ...
          const dateStr = eventDate ? eventDate.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'short' }) : '';
          const price = evt.min_price === 0 || evt.price === 0 ? 'Free' : evt.min_price ? `₹${evt.min_price}` : '';
          ...
          return (
            ...
            >
              ...
              >
                {evt.cover_image ? (
                  <Box component="img" src={evt.cover_image} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  icon
                )}
              </Box>
              <Box sx={{ p: 1 }}>
                ...
                <Typography sx={{ fontSize: 10, color: 'var(--color-text-secondary, #6b7280)', mt: 0.5 }}>
                  {dateStr}{dateStr && price ? ' · ' : ''}{price}
                </Typography>
              </Box>
            </Box>
          );
