...
const ComicIconButton = ({ children, onClick, active, color = 'white', activeColor = '#ef4444', sx = {} }: any) => (
    <IconButton
        onClick={onClick}
        sx={{
...
const HighlightInteractionsMobile = ({ highlight, onOpenComments }: { highlight: any; onOpenComments: () => void }) => {
...
export const HighlightFeedViewer = ({ highlights, isOpen, onClose, initialHighlightId }: HighlightFeedViewerProps) => {
    ...
    const handleNext = () => {
        if (activeIndex < highlights.length - 1) {
            ...
            window.history.replaceState(null, '', `/events/${eventId}/gallery/${nextHighlight.id}`);
        }
    };
    ...
    const handlePrev = () => {
        if (activeIndex > 0) {
            ...
            window.history.replaceState(null, '', `/events/${eventId}/gallery/${prevHighlight.id}`);
        }
    };
                        ...
                        >
                            ...
                            <Box sx={{
                                ...
                                <Box sx={{
                                    ...
                                    />
                                    <Typography variant="body1" sx={{ fontFamily: '"Caveat", cursive', fontSize: '1.4rem' }}>
                                        {h.text}
                                    </Typography>
                                </Box>
                            </Box>
        ...
        >
            ...
            <Box sx={{
                ...
                <Box sx={{
                    ...
                    <ComicIconButton onClick={handlePrev} sx={{ visibility: activeIndex > 0 ? 'visible' : 'hidden' }}>
                        <ChevronLeft />
                    </ComicIconButton>
                    <ComicIconButton onClick={handleNext} sx={{ visibility: activeIndex < highlights.length - 1 ? 'visible' : 'hidden' }}>
                        <ChevronRight />
                    </ComicIconButton>
                </Box>
            </Box>
