import { Image, PartyPopper, Star, XCircle } from 'lucide-react';
...
export function WrapUpStep({ event, readonly }: WrapUpStepProps) {
  ...
  const markEnabled =
    !readonly && isDraftOrPublished && event?.lifecycle_state !== 'event_ready';
  const cancelEnabled =
    !readonly && !isDraftOrPublished && event?.lifecycle_state !== 'event_ready';
  ...
  const handleCancel = async () => {
    if (
      !window.confirm(
        'Are you ABSOLUTELY sure you want to cancel this event? This cannot be undone.',
      )
    )
      return;
    try {
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border-2 border-gray-800 p-6 shadow-[3px_4px_0px_#333] relative">
          ...
          {/* Placeholder content */}
          <div
            className="mt-4 border-t-2 border-dashed border-gray-100 pt-4 text-center text-gray-400 italic"
