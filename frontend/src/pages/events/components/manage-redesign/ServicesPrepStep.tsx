...
export function ServicesPrepStep({ event, readonly }: ServicesPrepStepProps) {
    ...
    const [openVendorCard, setOpenVendorCard] = useState<number | null>(null);
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 border-b-2 border-gray-100 pb-2">
                            ...
                            <p className="text-sm text-gray-500 italic" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.2rem' }}>
                                <span className="font-bold">{filledNeedsCount}</span> of <span className="font-bold">{needs.length}</span> vendor services filled
                            </p>
                        </div>
                        {needs.length === 0 ? (
                            (event.features || []).length === 0 ? (
                                <p className="text-sm text-gray-400 italic py-4 text-center">You should add the needs you would need from vendors.</p>
                            ) : (
                            ...
                            needs.map((need) => (
                                <div key={need.id} className="group border-2 border-gray-50 hover:border-gray-200 rounded-xl p-4 transition-all bg-white shadow-sm hover:shadow-md">
                                    <div className="flex items-start gap-4">
                                        ...
                                        {openTooltip === need.id && (
                                            <TooltipForHostOverride need={need} tipNeedId={openTooltip} readonly={readonly} handleHostOverride={handleHostOverride} handleUndoOverride={handleUndoOverride} />
                                        )}
                                                ...
                                                >
                                                    ...
                                                    <p className="text-lg font-bold mt-1"
                                                        style={{
                                                            ...
                                                            color: need.status === 'override_filled' ? 'blue' : need.status === 'filled' ? 'green' : 'orange',

                                                        }}
                                                    >
                                                        {(need.status === 'filled') && (<><span className="font-bold">Filled by vendor</span></>)}
                                                        {(need.status !== 'filled' && need.status !== 'override_filled') && (<>• <span className="font-bold">{need.application_count}</span> applications</>)}
                                                        {(need.status === 'override_filled') && (<><span className="font-bold">Filled by host</span></>)}
                                                        {need.status === 'open' && (
                                                            <>
                                                                <Link
                                                                        ...
                                                                        <div key={app.id} className="p-4 bg-white border-2 border-gray-200 rounded-lg shadow-sm flex items-center justify-between gap-4">
                                                                            <div className="min-w-0">
                                                                                ...
                                                                                style={{ fontFamily: '"Permanent Marker", cursive' }}>
                                                                                    ...
                                                                                    <span className="text-[15px] text-gray-500">by </span>
                                                                                    <Hostname username={app.vendor_name} /> 
                                                                                    </p>
                                                                                
                                                                                <span className="text-[10px] text-green-600 font-bold mr-2">${app.proposed_price || '0'}</span>
                                                                                <span className="text-[10px] font-bold italic">{app.message }</span>
                                                                            </div>
                                                                            {app.status === 'pending' && (
                    ...
                    {(event.features || []).length > 0 && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 border-b-2 border-gray-100 pb-2">
                                ...
                                <h4 className="font-bold text-gray-700" style={{ fontFamily: '"Permanent Marker", cursive' }}>You are providing : </h4>
                            </div>
...
export function TooltipForHostOverride({ need, tipNeedId = 0, readonly = false, handleHostOverride, handleUndoOverride }: { need: EventNeed, tipNeedId?: number, readonly?: boolean, handleHostOverride: (needId: number) => Promise<void>, handleUndoOverride: (needId: number) => Promise<void> }) {
        ...
        return (
            <div className="relative">
                ...
                <div className="relative border-2 border-gray-900 bg-white shadow-[4px_4px_0px_#333] px-3 py-2 text-gray-900">
                    ...
                    <p className="text-[10px] uppercase tracking-[2px] text-gray-500 font-black">Need Filled by Vendor</p>
    ...
    return (
        <div className="relative">
            <div className="absolute top-0 left-0 z-10 max-w-[260px]">
                <div className="relative border-2 border-gray-900 bg-white shadow-[4px_4px_0px_#333] px-3 py-2 text-gray-900">
                    <div className="absolute -left-2 top-3 h-3 w-3 rotate-45 border-2 border-gray-900 bg-white" />
                    <p className="text-[10px] uppercase tracking-[2px] text-gray-500 font-black">Host Override</p>
                    <p className="text-xs text-gray-700 mt-1">
