...
const ManageDetailsSection = lazy(() => import('./manage/ManageDetailsSection').then(m => ({ default: m.ManageDetailsSection })));
const ManageAttendeesSection = lazy(() => import('./manage/ManageAttendeesSection').then(m => ({ default: m.ManageAttendeesSection })));
const ManageEntrySection = lazy(() => import('./manage/ManageEntrySection').then(m => ({ default: m.ManageEntrySection })));
const ManageNeedsSection = lazy(() => import('./manage/ManageNeedsSection').then(m => ({ default: m.ManageNeedsSection })));
...
export default function ManageEventPage() {
  ...
  const { data: eventResponse, isLoading: isLoadingEvent, refetch: refetchEvent } = useEvent(Number(id));
  const { data: attendeesResponse, isLoading: isLoadingAttendees, refetch: refetchAttendees } = useEventAttendees(
    Number(id),
  );
  const { data: lifecycleHistoryResponse } =
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="bg-white border-2 border-gray-800 p-5 shadow-[3px_4px_0px_#333] relative mb-6">
          <div className="flex items-center justify-between gap-1 mt-3 mb-4 overflow-x-auto pb-2">
            {FLOW_PIPELINE.map((state, idx) => {
              ...
              const canTransition = ALLOWED_TRANSITIONS[event.lifecycle_state as EventLifecycleState]?.includes(state as EventLifecycleState) && !isCurrent;
              ...
              return (
                <div key={state} className="flex items-center flex-shrink-0">
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ fontFamily: '"Caveat", cursive', fontSize: '0.85rem' }}>{cfg.label}</span>
                  </button>
          ...
          {selectedTransitionTarget && selectedTransitionTarget !== event.lifecycle_state && (
            <div className="border-2 border-dashed border-gray-400 rounded-lg p-4 bg-yellow-50/50 space-y-3 mb-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ backgroundColor: LIFECYCLE_FLOW_CONFIG[selectedTransitionTarget].bg, color: LIFECYCLE_FLOW_CONFIG[selectedTransitionTarget].color }}>
                  {LIFECYCLE_FLOW_CONFIG[selectedTransitionTarget].emoji} {LIFECYCLE_FLOW_CONFIG[selectedTransitionTarget].label}
                </span>
              </div>
              <input value={transitionReason} onChange={(e) => setTransitionReason(e.target.value)} placeholder="Reason (optional)" className="w-full rounded-lg border bg-white px-3 py-2 text-sm" />
              {selectedTransitionTarget === 'event_ready' && (
                <textarea value={eventReadyMessage} onChange={(e) => setEventReadyMessage(e.target.value)} rows={2} placeholder="Event Ready Message (required)" className="w-full rounded-lg border bg-white px-3 py-2 text-sm" />
              )}
              <div className="flex gap-2">
                <Button onClick={handleTransitionLifecycle} disabled={transitionLifecycle.isPending} size="sm" className="bg-yellow-300 text-gray-900 border-2 border-gray-800 shadow-[1px_2px_0px_#333]">Apply</Button>
                <Button variant="ghost" size="sm" onClick={() => setSelectedTransitionTarget(null)}>Cancel</Button>
              </div>
            </div>
          )}

          <button onClick={() => setShowHistory(!showHistory)} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-700">
            {showHistory ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
            <span style={{ fontFamily: '"Caveat", cursive', fontSize: '0.9rem' }}>{showHistory ? 'Hide' : 'Show'} transition history</span>
          </button>
            ...
            <div className="mt-3 pt-3 border-t border-gray-200 space-y-2">
              {lifecycleHistory.slice(0, 5).map((entry) => (
                <div key={entry.id} className="flex items-center gap-2 text-[10px]">
                  ...
                  <span className="ml-auto opacity-60">{new Date(entry.created_at).toLocaleDateString()}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex gap-1 mb-8 overflow-x-auto pb-1 mt-4">
          {[
            ...
            { id: 'attendees', icon: Users, label: `Attendees (${event.ticket_count || 0})`, path: 'attendees' },
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-2 px-5 py-2.5 border-2 border-b-0 transition-all whitespace-nowrap ${isActive
        ...
        {/* ═══ Content Area ═══ */}
        <Suspense fallback={<div className="p-8 text-center animate-pulse">Loading section...</div>}>
          <Routes>
            <Route
              index
              element={
            ...
            <Route
              path="needs"
              element={<ManageNeedsSection eventId={event.id} isSeries={!!event?.series} />}
            />
