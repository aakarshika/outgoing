...
type ChecklistItem = {
  ...
  due: string;
  /** 'host' = done but host responsible (blue); 'sales' = sales threshold (highlighted). */
  variant?: 'host' | 'sales';
};

function formatChecklistDeadline(daysUntilEvent: number | null) {
...
export default function PlanningWorkspacePage() {
  ...
  const checklistItems = useMemo<ChecklistItem[]>(() => {
    ...
    const salesPercentage =
      derivedCapacity > 0 ? (soldCount / derivedCapacity) * 100 : 0;
    ...
    const daysUntilEvent = startDate
      ...
      : null;
    const salesStatus: ChecklistItem['status'] = isSalesHealthy
      ? 'done'
      : daysUntilEvent !== null && daysUntilEvent <= 3
        ? 'warn'
        : 'todo';
    const needChecklistItems: ChecklistItem[] =
      eventNeeds.length === 0
        ? [
            {
        ...
        : eventNeeds.map((need) => {
            ...
            if (need.application_count > 0) {
              return {
                ...
                status:
                  daysUntilEvent !== null && daysUntilEvent <= 3 ? 'warn' : 'todo',
