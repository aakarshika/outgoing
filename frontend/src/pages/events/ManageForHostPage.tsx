...
export const HOST_STEPS = [
  ...
  },
  {
    ...
    routeSlug: 'publish',
    isHalfStep: true,
    components: [{ id: 'publish', component: PublishStep as any }],
  },
  {
    stepId: 2,
    title: 'Prepare',
    ...
    isHalfStep: false,
    components: [{ id: 'services-prep', component: ServicesPrepStep as any }],
  },
  {
    stepId: 2.5,
    title: 'Ready',
    ...
    isHalfStep: true,
    components: [{ id: 'event-readiness', component: EventReadinessStep as any }],
  },
  {
    stepId: 3,
    title: 'Live',
    ...
    isHalfStep: false,
    components: [{ id: 'live-attendance', component: LiveAttendanceStep as any }],
  },
  {
    stepId: 4,
    title: 'Wrap Up',
    routeSlug: 'wrap-up',
    isHalfStep: false,
    components: [{ id: 'wrap-up', component: WrapUpStep as any }],
  },
];
...
export default function ManageForHostPage() {
  ...
  const matchedStep = activeSlug
    ? HOST_STEPS.find((step) => step.routeSlug === activeSlug)
    : null;
  const hasValidSlug = Boolean(matchedStep);
  const activeStep =
    matchedStep ??
    HOST_STEPS.find((step) => step.routeSlug === defaultStepSlug) ??
    HOST_STEPS[0];
  const activeStepIndex = HOST_STEPS.findIndex(
    (step) => step.stepId === activeStep.stepId,
  );
  ...
  const canSaveBasicInfo =
    !event?.lifecycle_state || ['draft', 'published'].includes(event.lifecycle_state);

  const { data: occurrencesResponse, refetch: refetchOccurrences } =
  ...
  useEffect(() => {
    if (event) {
      ...
      if ((event.features?.length ?? 0) > 0)
        setEventFeatures((event.features ?? []) as EventFeature[]);
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (ticketTiers.length > 0 && totalCapacityVal !== null) {
      ...
      const sumTiers = manuallySetTiers.reduce(
    ...
    try {
      ...
      const tiersToSave = ticketTiers.map((t, index) => {
        ...
        let tierCap =
          t.capacity === '' || t.capacity === null ? null : Number(t.capacity);
  ...
  return (
    <div className="min-h-screen px-4 sm:px-6 py-8">
      <div className="mx-auto max-w-4xl">
        {!event || (user && user.username !== event.host.username) ? (
                ...
                <div className="mt-10 flex flex-wrap items-center justify-between gap-4  pt-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto mt-4 sm:mt-0">
                    <button
                      ...
                      className={`px-6 py-3 border-2 border-gray-900 font-bold uppercase tracking-wider transition-all 
                                        ${
                                          isSubmitting
                                            ? 'bg-gray-200 text-gray-500'
                                            : 'bg-yellow-300 shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:-translate-y-1 hover:shadow-[6px_6px_0_0_rgba(0,0,0,1)] active:translate-y-1 active:shadow-[2px_2px_0_0_rgba(0,0,0,1)]'
                                        }`}
                      style={{ fontFamily: '"Permanent Marker", cursive' }}
                    >
