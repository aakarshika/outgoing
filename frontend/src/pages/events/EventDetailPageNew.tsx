...
import { CategoricalBackground, CATEGORY_THEMES } from '@/features/events/CategoricalBackground';
...
const MapPlaceholder = ({ location }: { location: string }) => (
    ...
    >
        ...
        <Box sx={{
            ...
            <Typography sx={{ fontFamily: '"Permanent Marker"', fontSize: '0.9rem' }}>View on Maps</Typography>
            <Typography variant="caption" sx={{ display: 'block', mt: 0.5, px: 2, lineClamp: 1 }}>{location}</Typography>
        </Box>
...
const PolaroidFrame = ({ src, type = 'image', caption, author, rotation }: { src: string | null; type?: 'image' | 'video'; caption?: string; author?: string; rotation?: number }) => {
    ...
    return (
        <Paper elevation={3} sx={{
            ...
            <Box sx={{ aspectSquare: 1, bgcolor: '#f0f0f0', overflow: 'hidden', position: 'relative' }}>
                {type === 'video' ? (
                    <Media type="video" src={src || undefined} controls className="w-full h-full object-cover" />
                ) : (
...
const TicketStub = ({ type, price, onBuy, isLoading, hasTicket }: { type: string; price: number; onBuy: () => void; isLoading?: boolean; hasTicket?: boolean }) => (
    <Paper elevation={2} sx={{
        ...
        <Box sx={{
            ...
            <Typography variant="caption" sx={{ fontWeight: 'bold', color: 'text.secondary', letterSpacing: 1 }}>ADMIT ONE</Typography>
            <Typography variant="h5" sx={{ fontFamily: '"Permanent Marker"', color: 'primary.main', mt: 1 }}>${price}</Typography>
        </Box>
        <Box sx={{ p: 2, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <Typography variant="h6" sx={{ fontFamily: '"Permanent Marker"', fontSize: '1.2rem' }}>{type} Access</Typography>
            <Typography variant="caption" sx={{ display: 'block', mt: 0.5, opacity: 0.7 }}>Valid for one person. No refunds.</Typography>
            {!hasTicket ? (
                <MuiButton
                    variant="contained"
                    size="small"
                    onClick={onBuy}
                    disabled={isLoading}
                    sx={{ mt: 1.5, borderRadius: 0, bgcolor: '#333', '&:hover': { bgcolor: '#000' } }}
                >
                    {isLoading ? 'Processing...' : 'BUY TICKET'}
                </MuiButton>
            ) : (
                <Box sx={{ mt: 1.5, p: 0.5, border: '2px solid #16a34a', color: '#16a34a', textAlign: 'center', transform: 'rotate(-3deg)', fontWeight: 'bold', fontFamily: '"Permanent Marker"' }}>
                    PAID / ARCHIVED
                </Box>
            )}
        </Box>
...
const ClassifiedAd = ({ need, onInquire, isEligible = false, isOpportunity = false, navigate }: { need: any; onInquire: (n: any) => void; isEligible?: boolean; isOpportunity?: boolean; navigate: any }) => (
    <Box sx={{ position: 'relative', width: '100%' }}>
        ...
        >
            ...
            <Typography variant="body2" sx={{ fontFamily: 'serif', fontStyle: 'italic', mb: 2, lineHeight: 1.4 }}>
                {need.description}
            </Typography>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mt: 2 }}>
                <Box>
                    <Typography sx={{ fontSize: '0.7rem', color: '#666', mb: 0.5 }}>Criticality: {need.criticality}</Typography>
        ...
        {need.status === 'filled' && (
            <Box sx={{
                ...
                <Box sx={{ transform: 'rotate(-3deg) scale(0.95)', pointerEvents: 'auto', filter: 'drop-shadow(5px 5px 15px rgba(0,0,0,0.2))' }}>
                    <VendorBusinessCard
                        vendor={{
                    ...
                    />
                    <Box sx={{
                        position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%) rotate(5deg)',
                        width: 50, height: 18, bgcolor: 'rgba(59, 130, 246, 0.5)', borderRadius: '1px',
    ...
    const reviews = useMemo(() => {
        ...
        return baseReviews.map((rev: any) => ({
            ...
            vendorReviews: (rev.vendor_reviews || []).map((vRev: any) => {
                const vendorInfo = event?.participating_vendors?.find((v: any) => v.id === vRev.vendor_id);
                return {
    ...
    return (
        <ThemeProvider theme={scrapbookTheme}>
            <Box sx={{
                ...
                backgroundImage: event.lifecycle_state === 'live'
                ...
                <Box sx={{
                    ...
                    >
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 4 }}>
                            <IconButton onClick={() => navigate(-1)} sx={{ bgcolor: 'white', '&:hover': { bgcolor: '#f0f0f0' }, boxShadow: 1 }}>
                                <ArrowLeft />
                            </IconButton>
                            {isHost && (
                                <MuiButton variant="contained" startIcon={<FileEdit />} onClick={() => navigate(`/events/${event.id}/manage`)}
                                    sx={{ bgcolor: 'white', color: 'black', '&:hover': { bgcolor: '#f0f0f0' }, boxShadow: 2, textTransform: 'none', fontWeight: 'bold' }}>
                                    Manage Event
                                </MuiButton>
                            )}
                        </Box>
                            ...
                            <Grid size={{ xs: 12, md: 5 }}>
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    {(event.media || []).slice(0, 2).map((m: any, idx: number) => (
                                        <PolaroidFrame key={m.id} src={m.file} type={m.media_type} rotation={idx % 2 === 0 ? 3 : -4} />
                                    ))}
                                </Box>
                            </Grid>
                        </Grid>
                        ...
                        <Box sx={{ position: 'relative', mb: 6 }}>
                            ...
                            >
                                <WashiTape
                                    color={CATEGORY_THEMES[event.category?.slug || '']?.tape || "rgba(37, 99, 235, 0.4)"}
                                    rotate={event.category?.slug === 'comedy' ? '10deg' : '3deg'}
                                />
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 2, position: 'relative', zIndex: 1 }}>
                                    {event.category && (
                                        <Chip label={event.category.name} sx={{ bgcolor: 'primary.main', color: 'white', fontFamily: '"Permanent Marker"' }} />
                                    )}
                                    <Chip
                                        ...
                                        sx={{
                                            ...
                                            bgcolor: event.lifecycle_state === 'live' ? 'success.main' : 'transparent',
                                            color: event.lifecycle_state === 'live' ? 'white' : (event.category?.icon === 'cpu' ? '#fff' : 'inherit'),
                                            borderColor: event.category?.icon === 'cpu' ? '#fff' : 'inherit',
                                            boxShadow: event.lifecycle_state === 'live' ? '0 0 15px rgba(34, 197, 94, 0.5)' : 'none'
                                        }}
                                    />
                                ...
                                <Box sx={{ pr: { xs: 0, sm: '240px' } }}>
                                    ...
                                    <Grid container spacing={2} sx={{ color: 'inherit', opacity: 0.8, position: 'relative', zIndex: 1 }}>
                                        ...
                                        <Grid size="auto" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <Clock size={18} />
                                            <Typography>{new Date(event.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Typography>
                                        </Grid>
                                        {event.location_name && (
                                            <Grid size="auto" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                <MapPin size={18} />
                            ...
                            <Box sx={{
                                ...
                                <HostCard
                                    ...
                                    tag={event.category?.name === 'Photography' ? 'Photographer' : 'Vibe Architect'}
                                />
                            </Box>
                        </Box>
                        ...
                        <Grid container spacing={6}>
                            ...
                            <Grid size={{ xs: 12, md: 6 }}>
                                <Box sx={{ spaceY: 6 }}>
                                    {/* Description */}
                                    <Paper sx={{ p: 4, position: 'relative', bgcolor: (['utensils', 'book-open'].includes(event.category?.icon || '') ? '#fdf8f4' : '#fff'), color: 'inherit' }}>
                                        <WashiTape color="rgba(22, 163, 74, 0.3)" rotate="-2deg" />
                                        <Typography variant="h6" sx={{ fontFamily: '"Permanent Marker"', mb: 2 }}>The Details</Typography>
                                        <Typography sx={{ lineHeight: 1.8, whiteSpace: 'pre-wrap', mb: 4 }}>{event.description}</Typography>
                                        ...
                                        {event.location_name && (
                                            <Box sx={{ mt: 4 }}>
                                                <Typography variant="caption" sx={{ display: 'block', mb: 1, fontWeight: 'bold' }}>LOCATION</Typography>
                                                <MapPlaceholder location={event.location_address || event.location_name} />
                                            </Box>
                                        )}
                                    </Paper>
                                    ...
                                    {['published', 'at_risk', 'event_ready'].includes(event.lifecycle_state) && !event.user_has_ticket && !isHost && (
                                        <Box sx={{ mt: 6 }}>
                                            <Typography variant="h6" sx={{ fontFamily: '"Permanent Marker"', mb: 3 }}>Get Your Tickets</Typography>
                                            <TicketStub
                                                type="Standard"
                                    ...
                                    {displayNeeds.length > 0 && highlights.length > 0 && (
                                        <Box sx={{ mt: 6 }}>
                                            <Typography variant="h6" sx={{ fontFamily: '"Permanent Marker"', mb: 3 }}>Host is Looking For...</Typography>
                                            {displayNeeds.map((need: any) => {
                                                const myServices = myServicesResponse?.data || [];
                                                const isEligible = myServices.some((s: any) =>
                            ...
                            <Grid size={{ xs: 12, md: 6 }} sx={{ display: 'flex', flexDirection: 'column' }}>
                                ...
                                {(!['live', 'completed'].includes(event.lifecycle_state) || highlights.length === 0) && (
                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                        ...
                                        {displayNeeds.length > 0 && highlights.length === 0 && (
                                            <Box>
                                                <Typography variant="h6" sx={{ fontFamily: '"Permanent Marker"', mb: 3, textAlign: 'center' }}>Host is Looking For...</Typography>
                                                {displayNeeds.map((need: any) => {
                                                    const myServices = myServicesResponse?.data || [];
                                                    const isEligible = myServices.some((s: any) =>
                                                        s.category.toLowerCase().includes(need.category.toLowerCase()) ||
                                ...
                                {['live', 'completed'].includes(event.lifecycle_state) && (
                                    <Box sx={{ mt: highlights.length === 0 ? 6 : 0 }}>
                                        <Typography variant="h2" sx={{ mb: 4, textAlign: 'center' }}>Memory Box</Typography>
                                        ...
                                        {event.lifecycle_state === 'completed' && (
                                            <Box>
                                                <Typography variant="h3" sx={{ mb: 4 }}>What people said</Typography>
                                                {reviews.length > 0 ? (
                                                    ...
                                                    <Typography sx={{ fontStyle: 'italic', color: 'text.secondary', textAlign: 'center' }}>No sticky notes yet.</Typography>
                                                )}
