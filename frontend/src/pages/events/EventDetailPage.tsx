    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      ...
      <div className="mb-8">
        <EventCarousel
          items={[
            ...(event.cover_image
              ? [{ id: 'cover', url: event.cover_image, type: 'image' as const }]
              : []),
            ...
            ...((event.highlights || [])
              .map((h: any) =>
                h.media_file
                  ? { id: `h-${h.id}`, url: h.media_file, type: 'image' as const }
                  : null,
              )
      ...
      <div className="mb-8">
        ...
        <span
          className={`inline-block rounded-full px-3 py-1 text-xs font-semibold mb-2 ${LIFECYCLE_STYLES[event.lifecycle_state]}`}
        >
      ...
      {['live', 'completed'].includes(event.lifecycle_state) && (
        <div className="mt-12 space-y-12">
          ...
          <section className="space-y-6 pt-12 border-t border-border/50">
            ...
            {(event.highlights?.length ?? 0) > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {event.highlights!.map((highlight: any) => (
                  <div
                    key={highlight.id}
                    className="aspect-square bg-muted rounded-xl relative overflow-hidden group"
                  >
            ...
            <Button variant="outline" onClick={() => setIsHighlightOpen(true)}>
              Add a Highlight
            </Button>
          </section>
            ...
            <section className="space-y-6 pt-6 border-t border-border/50">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">
                              {review.reviewer_username || 'Anonymous User'}
                            </p>
                      ...
                      {review.media?.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto pb-2 snap-x">
                          {review.media.map((media: any) => (
                            <div
                              key={media.id}
                              className="relative w-32 h-32 rounded-lg shrink-0 overflow-hidden bg-muted snap-start"
                            >
                        ...
                        <div className="mt-4 pt-4 border-t space-y-3">
                          ...
                          <div className="grid sm:grid-cols-2 gap-3">
                            {review.vendor_reviews.map((vr: any) => (
                              <div
                                key={vr.id}
                                className="bg-muted/30 rounded-xl p-3 border border-border/50 flex flex-col gap-2"
                              >
                                <div className="flex items-center justify-between">
              ...
              <div className="bg-primary/5 rounded-2xl p-6 border border-primary/10 flex flex-col items-center justify-center text-center space-y-3 mt-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Share your experience to help others and support the host.
                </p>
                <Button
