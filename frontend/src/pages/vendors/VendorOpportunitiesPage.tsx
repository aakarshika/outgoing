...
import { useMyVendorOpportunities, useMyPotentialOpportunities } from '@/features/needs/hooks';
...
const CRITICALITY_STYLES: Record<string, { border: string; text: string; bg: string }> = {
    ...
    non_substitutable: { border: '#ea580c', text: '#ea580c', bg: 'rgba(234, 88, 12, 0.06)' },
...
const WashiTape = ({ color = 'rgba(251, 191, 36, 0.5)', rotate = '3deg', width = 80 }) => (
    <Box sx={{
        position: 'absolute',
        top: -10,
        left: '50%',
...
function NeedCard({ opportunity, isPotential = false, onApply }: NeedCardProps) {
    ...
    const critStyle = CRITICALITY_STYLES[opportunity.criticality] || CRITICALITY_STYLES.replaceable;
            ...
            >
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1, mb: 1.5 }}>
                    ...
                    <Box sx={{
                        ...
                        <Typography sx={{ fontSize: '0.6rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                        <Calendar size={12} />
                        <Typography sx={{ fontSize: '0.7rem', fontFamily: 'monospace', color: '#555' }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                        <DollarSign size={12} />
                        <Typography sx={{ fontSize: '0.75rem', fontWeight: 'bold', fontFamily: 'monospace' }}>
                            {formatBudget(opportunity.budget_min, opportunity.budget_max)}
                        </Typography>
                    </Box>
                    <Typography sx={{ fontSize: '0.7rem', fontWeight: 'bold', fontFamily: '"Permanent Marker", cursive', color: '#ea580c' }}>
                ...
                <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
                    <Button variant="outline" size="sm" asChild>
                        <Link to={`/events/${opportunity.event_id}`} style={{ textDecoration: 'none' }}>View Event</Link>
                    </Button>
                    {isPotential ? (
                        <Button
                            size="sm"
