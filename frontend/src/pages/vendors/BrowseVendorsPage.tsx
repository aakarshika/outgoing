    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      {inviteFlowEnabled && (
        <div className="mb-6 rounded-xl border bg-primary/5 px-4 py-3 text-sm">
          <span className="font-medium">Invite mode:</span> Sending invites for{' '}
          <span className="font-medium">{inviteNeedTitle || 'selected need'}</span>.
        </div>
      )}
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="mt-3 flex gap-2">
                  <Button variant="outline" size="sm" asChild className="flex-1">
                    <Link to={`/vendors/portfolio/${service.vendor_id}`}>
                      View Portfolio
                    </Link>
                  </Button>
                  {inviteFlowEnabled && (
                      ...
                      onClick={async () => {
                        try {
                          ...
                          setInvitedVendorIds((prev) =>
                            new Set(prev).add(service.vendor_id),
                          );
