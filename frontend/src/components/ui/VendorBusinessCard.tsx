...
export const VendorBusinessCard = ({ vendor, rotation = 0, onClick }: VendorBusinessCardProps) => {
    ...
    return (
        <Box sx={{
            ...
            '&:hover': onClick ? { transform: `rotate(${rotation}deg) translateY(-5px)`, boxShadow: '5px 5px 15px rgba(0,0,0,0.15)' } : {},
            '&::before': {
                content: '""',
                position: 'absolute',
                inset: 4,
            ...
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                <Box sx={{
                    width: 60, height: 60, bgcolor: '#ddd', flexShrink: 0,
                    border: '1px solid #ccc', overflow: 'hidden', boxShadow: 'inset 0 0 5px rgba(0,0,0,0.1)',
                    position: 'relative'
                }}>
                    {image ? (
                        <img src={image} alt={serviceName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                        <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#999', fontSize: '1.5rem' }}>👤</Box>
                    )}
                </Box>
                <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography sx={{
                        fontFamily: '"Lora", serif', fontWeight: 700, fontSize: '1.1rem', color: '#1a1a1a', lineHeight: 1.2, mb: 0.5,
                    ...
                    <Typography sx={{
                        fontFamily: '"Permanent Marker"', fontSize: '0.75rem', color: '#d97706', textTransform: 'uppercase', mb: 1
                    }}>
                        {category} Specialist
                    </Typography>
                    <Typography sx={{ fontSize: '0.6rem', color: '#888', fontFamily: 'monospace' }}>
                        EST. {year} • {events} HIRES
                    </Typography>
                </Box>
            </Box>
            <Box sx={{ mt: 'auto', borderTop: '1px solid rgba(0,0,0,0.05)', pt: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <Box>
                    <Typography sx={{ fontSize: '0.65rem', color: '#888', fontStyle: 'italic' }}>Verified Outgoing™ Vendor</Typography>
                ...
                <Box sx={{
                    ...
                    <Typography sx={{ fontSize: '0.5rem', fontWeight: 'bold', textAlign: 'center' }}>OFFICIAL<br />SEAL</Typography>
                </Box>
            </Box>
        </Box>
    );
