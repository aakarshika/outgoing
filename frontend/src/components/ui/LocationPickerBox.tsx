...
export function LocationPickerBox({ onChange }: LocationPickerBoxProps) {
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      const shortLabel =
        parts.slice(0, 3).join(',').trim() || result?.venueName || 'Current Location';
    ...
    >
      ...
      <Box
        ...
        sx={{
          ...
          px: 2,
          py: 0.7,
          bgcolor: selected ? '#fcd34d' : '#fff',
          border: '2px solid #333',
          boxShadow: selected ? 'inset 0 2px 4px rgba(0,0,0,0.08)' : '2px 3px 0px #333',
          cursor: 'pointer',
      ...
      >
        <MapPin
          size={14}
          strokeWidth={2.5}
          style={{ flexShrink: 0, color: selected ? '#1a1a1a' : '#ef4444' }}
        />
        <Typography
          noWrap
          sx={{
