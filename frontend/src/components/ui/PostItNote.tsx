...
export const PostItNote = ({
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ position: 'absolute', top: -5, left: '45%', width: 40, height: 15, bgcolor: 'rgba(0,0,0,0.1)' }} />
            ...
            {vendorReviews.length > 0 && (
                <Box sx={{
                    ...
                    <Typography sx={{ fontSize: '0.65rem', fontWeight: 'bold', color: 'text.secondary', letterSpacing: 0.5 }}>VENDOR SHOUTOUTS</Typography>
                    {vendorReviews.map((v, i) => (
                        <Box key={i} sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
                            <UserAvatar src={v.vendorAvatar} username={v.vendorName} size="xs" />
                            <Box sx={{ flex: 1 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Typography variant="caption" sx={{ fontWeight: 'bold' }}>{v.vendorName}</Typography>
                                    <Box sx={{ display: 'flex', color: '#d4af37' }}>
                                        {[...Array(5)].map((_, si) => (
                                            <Star key={si} size={8} fill={si < v.rating ? "currentColor" : "none"} />
                                        ))}
                                    </Box>
                                </Box>
                                {v.comment && (
