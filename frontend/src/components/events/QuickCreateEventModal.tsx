...
import { canUseBrowserGeolocation, getCurrentCoordinates, reverseGeocodeCoordinates } from '@/utils/geolocation';
...
export function QuickCreateEventModal({
    ...
    const submitEvent = async (status: 'draft' | 'published', advancedOptions: boolean = false) => {
        ...
        try {
            ...
            if (formObj) {
                const startVal = (formObj.elements.namedItem('start_time') as HTMLInputElement)?.value;
                const endVal = (formObj.elements.namedItem('end_time') as HTMLInputElement)?.value;
    ...
    const startDateVal = (formRef.current?.elements.namedItem('start_time') as HTMLInputElement)?.value;
    const hasDate = !!startDateVal;
    const isLocationValid = locationMode === 'online' ? true : !!(locationAddressRef.current && locationAddressRef.current.value.trim().length > 0);

    const canSave = title.trim().length > 0 && category !== '' && description.trim().length > 0;
            ...
            <DialogContent className="max-w-4xl border-none bg-[#f4f2f9ff] shadow-none w-[95vw]">
                <form id="quick-create-form">
                    <DialogHeader className="sr-only">
                        ...
                        <DialogDescription>Quickly create a new event draft or publish immediately.</DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="max-h-[85vh] overflow-y-auto px-1 py-1 hide-scrollbar">
                        ...
                        <div className="flex items-start gap-2 text-sm text-gray-800 mt-4 mr-2 bg-yellow-100 p-3 rounded-md border-2 border-gray-800 shadow-[2px_2px_0px_#333]">
                            ...
                            <div style={{ fontFamily: '"Permanent Marker", cursive' }} className="leading-snug">
                                Access the events you manage from <span className="text-black bg-white px-1 border border-gray-800 rounded mx-1"><Menu size={14} className="inline mb-0.5" /> Menu</span> &gt; <span className="text-black highlight-text inline-block">My Events</span>.
                                <br />
                                Next steps: <span className="text-black underline decoration-wavy decoration-2 decoration-[#AF90F9] underline-offset-4">Prepare &gt; Live &gt; Done</span>
                            </div>
                        </div>
