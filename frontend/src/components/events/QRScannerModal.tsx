...
import { Camera, CheckCircle2, RefreshCw, Smartphone, Ticket as TicketIcon, X, XCircle, ShieldAlert } from 'lucide-react';
        ...
        try {
            ...
            await html5QrCodeRef.current.start(
                ...
                (errorMessage) => {
                    ...
                    if (Math.random() < 0.005) console.log('[QRScanner] Frame error:', errorMessage);
                }
            );
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black flex flex-col pt-[max(env(safe-area-inset-top),20px)]">
            ...
            <div className="flex justify-between items-center px-6 py-4 text-white shrink-0 z-50">
                <h2 className="text-xl flex items-center gap-2" style={{ fontFamily: '"Permanent Marker"' }}>
            ...
            {!isSecure && (
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-900">
                    ...
                    <h3 className="text-2xl font-black text-white mb-2 uppercase tracking-wide">Secure Connection Required</h3>
                    <p className="text-gray-400 mb-6">Browsers only allow camera access over <b>HTTPS</b> or <b>localhost</b>.</p>
                    <div className="bg-red-500/10 border-2 border-red-500/30 p-4 rounded-lg text-red-400 text-sm font-mono mb-6 break-all">
                        https://{window.location.host}
                    </div>
                    <p className="text-sm text-gray-500 italic">Please use the exact URL shown above starting with <b>https://</b></p>
                </div>
            )}
            ...
            {isSecure && hasPermission === false && (
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-900">
                    ...
                    <h3 className="text-2xl font-black text-white mb-2 uppercase tracking-wide">Camera Blocked</h3>
                    <p className="text-gray-400 mb-6">Please allow camera access in your browser settings to scan tickets.</p>
                    <button
                    ...
                    {isProcessing && !scanResult && !scanError && (
                        <div className="absolute inset-0 flex items-center justify-center z-30">
                            <div className="bg-black/60 backdrop-blur-sm px-6 py-4 rounded-2xl border-2 border-white/20 flex items-center gap-3">
                                ...
                                <span className="text-white font-bold tracking-widest text-lg">VERIFYING...</span>
                            </div>
                        </div>
                    )}
                            ...
                            >
                                {scanError ? (
                                    <div className="text-center py-4">
                                        ...
                                        <h3 className="text-2xl font-black text-gray-900 mb-2 uppercase tracking-wide">Invalid Code</h3>
                                        <p className="text-gray-700 font-bold mb-8 px-4 leading-tight">{scanError}</p>
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-gray-500 font-bold mb-6 text-sm uppercase tracking-widest">@{scanResult.attendee_username}</p>
                                        ...
                                        <div className="bg-gray-50 p-5 rounded-2xl mb-8 flex items-center gap-4 text-left border-2 border-gray-100 shadow-inner">
                                            ...
                                            <div>
                                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] mb-1">Pass Access</p>
                                                <p className="font-black text-gray-900 text-xl leading-none">{scanResult.ticket_type}</p>
                                            </div>
                                        </div>
            ...
            {isSecure && hasPermission !== false && !scanResult && !scanError && (
                <div className="shrink-0 p-8 text-center text-gray-400">
                    <p className="text-sm font-medium tracking-wide mb-1" style={{ fontFamily: '"Caveat", cursive', fontSize: '1.2rem' }}>
