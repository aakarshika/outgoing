      ...
      <div className="border-t pt-6 mb-6">
        ...
        <div className="space-y-4">
          {displayNeeds.map((need: any) => {
            const matchingServices = myServices.filter(
              (s: any) => s.category === need.category,
            );
            const hasMatchedService = matchingServices.length > 0;
            const hasAssignedVendor = need.assigned_vendor !== null;
            const userApplication = need.applications?.find((app: any) =>
              myServices.some((s: any) => s.id === app.service),
            );
            ...
            const acceptedApp = need.applications?.find(
            ...
            return (
              <div
                key={need.id}
                className="bg-card border rounded-xl p-4 flex flex-col sm:flex-row justify-between gap-4"
              >
                <div>
                  <h3 className="font-semibold">{need.title}</h3>
                  <div className="flex gap-2 mt-1 mb-2 text-xs">
                    <span
                      className={`px-2 py-0.5 rounded capitalize ${need.status === 'filled' ? 'bg-primary/10 text-primary font-medium' : 'bg-muted text-muted-foreground'}`}
                    >
                      {need.status === 'filled'
                        ? 'Filled'
      ...
      return (
        <div className="rounded-xl border bg-card p-6 space-y-4 sticky top-24 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Ticket sales are no longer active for this event.
          </p>
        </div>
      );
    }
      ...
      <div className="border rounded-xl p-4 bg-primary/5 border-primary/20 mb-6">
        ...
        <p className="text-sm text-foreground whitespace-pre-wrap">
          {event.check_in_instructions ||
            'Check-in instructions will be shared by the host soon.'}
        </p>
        {event.event_ready_message && (
          <div className="mt-4 pt-4 border-t border-primary/10">
            <p className="text-sm italic text-muted-foreground">
              Host message: {event.event_ready_message}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="lg:col-span-2">
        ...
        {(state === 'draft' ||
          ...
          <>
            ...
            <Description />
            {event.series && (
              <EventSeriesNav occurrences={occurrences} currentEventId={event.id} />
            )}
            <NeedsList />
          </>
        )}
          ...
          <>
            ...
            {!canViewReadyDetails && (
              <div className="p-4 bg-muted text-muted-foreground rounded-xl mb-6 text-sm">
                Event day location details and check-in instructions are visible to paid
                attendees.
              </div>
            )}
          ...
          <>
            <div className="p-6 bg-card border rounded-2xl mb-8">
              ...
              <p className="text-muted-foreground mb-6">
                Thank you for attending. Relive the moments and share your feedback.
              </p>
            </div>
          ...
          <>
            <div className="bg-red-500/10 border-red-500/20 text-red-600 dark:text-red-400 border p-6 rounded-2xl mb-6">
              ...
              <p className="text-red-600/80">
                We apologize, but this event has been cancelled by the host.
              </p>
              ...
              <p className="text-sm mt-1 bg-white/50 dark:bg-black/20 p-3 rounded-lg">
                Unforeseen circumstances. Refunds have been initiated automatically if
                applicable.
              </p>
            </div>
