...
export function ApplyToNeedModal({
  isOpen,
  onClose,
  needId,
  needTitle,
}: ApplyToNeedModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg relative">
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(to right, rgba(0,0,0,0.02) 1px, transparent 1px), linear-gradient(transparent 95%, #e5e7eb 100%)',
              ...
              >
                ...
                {myServices.map((service: any) => (
                  <option key={service.id} value={service.id}>
                    {service.title}{' '}
                    {service.base_price ? `(from $${service.base_price})` : ''}
                  </option>
                ))}
              </select>
              {myServices.length === 0 && (
                ...
                >
                  You haven't created any services yet. You can still apply without
                  attaching one.
                </p>
              )}
            </div>
