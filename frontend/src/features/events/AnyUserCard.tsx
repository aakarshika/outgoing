...
/** Popover: pass open, anchorEl, onClose. Standalone strip/card: pass userId only. */
export type AnyUserCardProps =
  | { userId: number }
  | {
      userId: number;
      open: boolean;
...
export type FriendAvatarPopoverProps = Extract<AnyUserCardProps, { open: boolean }>;
...
export function AnyUserCard(props: AnyUserCardProps) {
  const { userId } = props;
  const isPopoverMode = 'open' in props;
  const open = isPopoverMode ? props.open : undefined;
  const anchorEl = isPopoverMode ? props.anchorEl : undefined;

  const { user, isAuthenticated } = useAuth();
  const isActive = isPopoverMode ? Boolean(open && anchorEl) : Boolean(userId);
  const fetchFriendships = Boolean(isActive && isAuthenticated && user?.id);
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    enabled: isActive && Boolean(userId),
  });
