        ...
        <Box sx={{ py: 6, position: 'relative', overflow: 'hidden' }}>
            <Box sx={{
                ...
                <Box sx={{ position: 'relative' }}>
                    <Box sx={{ position: 'absolute', top: -10, left: -10, width: 40, height: 20, bgcolor: 'rgba(251, 191, 36, 0.4)', transform: 'rotate(-5deg)', zIndex: 0 }} />
            ...
            <Box sx={{ position: 'relative' }}>
                ...
                {showLeftChevron && (
                    <Box sx={{
                        ...
                        width: 80, display: { xs: 'none', md: 'flex' }, alignItems: 'center',
                        ...
                        <Button
                            ...
                            sx={{
                                ...
                                bgcolor: '#fff', boxShadow: 2, ml: 2, pointerEvents: 'auto',
                                color: '#000', '&:hover': { bgcolor: '#f0f0f0' }
                            }}
                        >
                            <ChevronLeft size={24} />
                        </Button>
                ...
                <Box
                    ...
                    sx={{
                        ...
                        position: 'relative',
                        '&::before': {
                            content: '""', position: 'absolute', top: '50%', left: 0, right: 0, height: 1,
                            backgroundImage: 'linear-gradient(to right, rgba(0,0,0,0.1) 50%, transparent 50%)',
                            backgroundSize: '10px 1px', zIndex: -1
                        }
                    }}
                >
                    {isLoading
                        ...
                        : events.map((event, index) => {
                            const showCustomCard = (index + 1) % 3 === 0 && index !== events.length - 1;
                            const cycleIndex = Math.floor(index / 3) % 5; // 5 types of custom cards

                            return (
                                <Box key={`group-${event.id}`} sx={{ display: 'flex', gap: { xs: 3, sm: 4, md: 6 } }}>
                                    <Box sx={{ width: { xs: 240, sm: 280, md: 320 }, flexShrink: 0, scrollSnapAlign: 'start' }}>
                ...
                {showRightChevron && (
                    <Box sx={{
                        ...
                        <Button
                            ...
                            sx={{
                                minWidth: 40, width: 40, height: 40, borderRadius: '50%',
                                bgcolor: '#fff', boxShadow: 2, mr: 2, pointerEvents: 'auto',
                                color: '#000', '&:hover': { bgcolor: '#f0f0f0' }
                            }}
                        >
