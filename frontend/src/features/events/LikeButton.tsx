...
export const LikeButton = ({
    ...
    const handleInterestClick = (e: React.MouseEvent) => {
        ...
        setInterestCount((prev) =>
            newIsInterested ? prev + 1 : Math.max(0, prev - 1)
        );
    ...
    return (
        ...
        >
            <Heart
                size={16}
                className={`transition-colors ${isInterested ? 'fill-red-500 text-red-500' : 'text-gray-500'
                    }`}
            />
            {interestCount > 0 && (
                <span
                    className={`text-xs font-semibold pr-1 ${isInterested ? 'text-red-500' : 'text-gray-600'
                        }`}
                >
                    {interestCount}
                </span>
