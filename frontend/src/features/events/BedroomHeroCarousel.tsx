...
import { Calendar, MapPin, ArrowRight, ChevronLeft, ChevronRight, Heart } from 'lucide-react';
...
const FairyLightSVG = ({ x, y, delay }: { x: number; y: number; delay: number }) => (
    <g transform={`translate(${x}, ${y})`}>
        <circle r="6" fill="#fff7a1" style={{
            animation: `blink ${2 + Math.random() * 2}s infinite ease-in-out`,
            animationDelay: `${delay}s`,
            filter: 'drop-shadow(0 0 5px #fff7a1)'
        }} />
        <circle r="10" fill="transparent" stroke="#ffcc00" strokeWidth="0.5" opacity="0.3" />
    </g>
);
...
const EventTapeCard = ({ event, isFocused, index }: { event: EventListItem; isFocused: boolean; index: number }) => {
    const rotation = useMemo(() => (index % 2 === 0 ? 1 : -1) * (1 + Math.random() * 2), [index]);
    ...
    return (
        <Paper
            ...
            sx={{
                ...
                textDecoration: 'none',
                color: 'inherit',
                transform: isFocused ? `rotate(0deg) scale(1.15)` : `rotate(${rotation}deg) scale(0.95)`,
                ...
                '&:hover': {
                    transform: isFocused ? `rotate(0deg) scale(1.18)` : `rotate(0deg) scale(1.02)`,
                    zIndex: 35,
        ...
        >
            ...
            <Box sx={{ flexGrow: 1, overflow: 'hidden', position: 'relative', mb: 2, bgcolor: '#f5f5f5', borderRadius: '2px' }}>
                <Media
                ...
                <Box sx={{
                    ...
                    <Typography sx={{ fontWeight: 'bold', fontSize: '0.75rem', letterSpacing: '0.05em' }}>
                ...
                >
                    <Heart
                        size={18}
                        className={`transition-colors ${event.user_is_interested
                ...
                <Box sx={{ position: 'absolute', bottom: 10, left: 10, right: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                    {event.lifecycle_state === 'event_ready' && (
                        <Box sx={{
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                <Typography variant="caption" sx={{ fontWeight: 'bold', color: 'primary.main' }}>CAPACITY FILLED</Typography>
                                <Typography variant="caption" sx={{ fontWeight: 'bold' }}>{Math.round((event.ticket_count / (event.capacity || 100)) * 100)}%</Typography>
                            </Box>
                            <Box sx={{ width: '100%', height: 4, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 2, overflow: 'hidden' }}>
                                <Box sx={{ width: `${Math.min(100, (event.ticket_count / (event.capacity || 100)) * 100)}%`, height: '100%', bgcolor: 'primary.main', borderRadius: 2 }} />
                            </Box>
                        </Box>
                    )}
                    ...
                    {(event.lifecycle_state === 'published' || event.lifecycle_state === 'live') && (
                        <Box sx={{
                            ...
                            ${event.ticket_price_standard ? parseFloat(event.ticket_price_standard).toFixed(0) : 'Free'}
                        </Box>
                    )}
                </Box>
            </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 'auto' }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        <Typography variant="caption" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, fontWeight: 'bold' }}>
                            <Calendar size={12} /> {new Date(event.start_time).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </Typography>
                        <Typography variant="caption" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, opacity: 0.7 }}>
                ...
                {isFocused && event.reviews && event.reviews.length > 0 && (
                    <Box sx={{ mt: 2, pt: 2, borderTop: '1px dashed #ddd' }}>
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'start' }}>
                            <Avatar src={event.reviews[0].reviewer_avatar} sx={{ width: 28, height: 28, border: '1px solid #eee' }} />
                            <Box sx={{ flex: 1 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 'bold' }}>{event.reviews[0].reviewer_username}</Typography>
                                    <Rating value={event.reviews[0].rating} readOnly size="small" sx={{ fontSize: '0.65rem' }} />
                                </Box>
                                <Typography sx={{
                                    fontSize: '0.7rem',
                                    fontStyle: 'italic',
...
const CustomCardCarouselWrapper = ({ cycleIndex, isFocused, index }: { cycleIndex: number; isFocused: boolean; index: number }) => {
    const rotation = useMemo(() => (index % 2 === 0 ? 1 : -1) * (1 + Math.random() * 2), [index]);
    ...
    return (
        ...
        >
            <PhotoClip />
            <Box sx={{ width: '100%', height: '100%', '& > *': { width: '100%', height: '100%', m: 0 } }}>
...
export function BedroomHeroCarousel() {
    ...
    const mixedEvents = useMemo(() => {
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            if ((i + 1) % 3 === 0 && i !== events.length - 1) { // Every 4th card
    ...
    const displayEvents = useMemo(() => {
        ...
        return tripled.map((item): MixedCarouselItem => item.type === 'custom' ? { type: 'custom', index: Math.floor(Math.random() * 5) } : item);
    }, [mixedEvents]);
    ...
    useEffect(() => {
        ...
        const cardNodes = Array.from(container.querySelectorAll('[data-card="true"]')) as HTMLElement[];
    ...
    useEffect(() => {
        ...
        if (centeredIndex <= Math.floor(mixedEvents.length / 2)) {
            timeout = setTimeout(() => {
                ...
                const containerCenter = container.offsetWidth / 2;
                const nodeCenter = node.offsetLeft + (node.offsetWidth / 2);
                container.scrollTo({ left: nodeCenter - containerCenter, behavior: 'instant' } as any);
    ...
    return (
        <Box sx={{
            ...
            <IconButton
                ...
                sx={{ position: 'absolute', left: { xs: 0, lg: -40 }, top: '50%', zIndex: 100, bgcolor: 'white', boxShadow: 3, '&:hover': { bgcolor: '#f0f0f0' } }}
            >
                <ChevronLeft />
            </IconButton>
            <IconButton
                onClick={() => setCenteredIndex(prev => prev + 1)}
                sx={{ position: 'absolute', right: { xs: 0, lg: -40 }, top: '50%', zIndex: 100, bgcolor: 'white', boxShadow: 3, '&:hover': { bgcolor: '#f0f0f0' } }}
            >
                <ChevronRight />
            </IconButton>
            ...
            >
                ...
                <Box sx={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none', display: 'flex' }}>
