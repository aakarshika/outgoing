...
import type { VendorServiceDetailResponse, VendorServiceListResponse } from '@/types/vendors';
...
export async function createVendorService(formData: FormData) {
    const { data } = await client.post<VendorServiceDetailResponse>('/vendors/', formData, {
...
export async function updateVendorService(serviceId: number, formData: FormData) {
    const { data } = await client.patch<VendorServiceDetailResponse>(`/vendors/${serviceId}/`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
}
